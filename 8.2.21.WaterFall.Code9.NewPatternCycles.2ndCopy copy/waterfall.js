...
function waterfall() { 
  ...
  function oppositeRow(rowIndex){
    ...
    //matrix that holds the colors
      ...
      (squarestartX, squarestartY, squarestartX, squarestartY + lineLength, myColor, lineWidth);
  ...
  function squareColor(squareRow,squareColumn){
    if(patternNumber == 0){
    return generateTriangle(squareRow, squareColumn);
    }
    if(patternNumber == 1) {
    return generateVerticals(squareRow,squareColumn);
    }
    if(patternNumber == 2){ 
    return generateDiagonal(squareRow,squareColumn);
    }
    if(patternNumber == 3){ 
    return leftToright(squareRow,squareColumn);
    }
      if(patternNumber == 4){ 
    return rightToleft(squareRow,squareColumn);
      }
    }
  ...
  function randomLine(x1, y1, x2,y2, color, width){
    stroke(color);
    strokeWeight(width);
    shift = sizeRandomized * lineLength;
